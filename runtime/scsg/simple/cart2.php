...
<body class="second" >
	<div class="brand_list container_2">
		<div class="header">
			<a href="<?php echo IUrl::creatUrl("/");?>" class="logo"><img src="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/css/reg/loginlogo.png";?>" height="70" alt=""></a>
			<ul class="shortcut">
				...
				<li><a href="<?php echo IUrl::creatUrl("/seller/index");?>">商家管理</a></li>
		   		<li class='last'><a href="<?php echo IUrl::creatUrl("/site/help_list");?>">帮助中心</a></li>
			</ul>

			<p class="loginfo">
			<?php if($this->user){?>
			<?php echo isset($this->user['show'])?$this->user['show']:"";?>您好，欢迎您来到<?php echo $siteConfig->name;?>购物！[<a href="<?php echo IUrl::creatUrl("/simple/logout");?>" class="reg">安全退出</a>]
			<?php }else{?>
...
<link rel="stylesheet" type="text/css" href="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."//fap/css/zengzhi.css";?>">
...
jQuery(function(){
	...
	//初始化自提点数据(自添)
	takeselfInit("<?php echo isset($this->custom['takeself'])?$this->custom['takeself']:"";?>");
	...
	$('[name="takeself"]').val(id);
		if(id>0){
			$.ajax({
			  dataType: "json",
			  async:false,
			  url: '<?php echo IUrl::creatUrl("block/getOneTakeself");?>',
			  data: {'id':id},
...
function get_delivery()
{
	...
	$('[id^="deliveryFeeBox_"]').each(function(i)
	{
		...
		$.getJSON("<?php echo IUrl::creatUrl("/block/order_delivery");?>",{"province":province,"distribution":delivery,"goodsId":dataArray[1],"productId":dataArray[2],"num":dataArray[3]},function(content){
			//地区无法送达
...
function selectTakeself(deliveryId)
{
	...
	art.dialog.open("<?php echo IUrl::creatUrl("/block/takeself");?>"+paramStr,{
		title:'选择自提点',
		okVal:'选择',
	...
	<form action='<?php echo IUrl::creatUrl("/simple/cart3");?>' method='post' name='order_form' callback='orderFormInstance.isSubmit();'>
			...
			<div class="cont">
				...
				<div class="wrap_box">
					...
					<table class="form_table" id="address_show_box" style='display:none'>
						...
						<script type='text/html' id='addressShowTemplate'>
							<tr><th>收货人信息：</th><td><%=province_val%> <%=city_val%>  (<%=accept_name%>收)</td></tr>
							<tr><th></th><td><%=area_val%> <%=address%>  <%=mobile%>  <%=telphone%></td></tr>
							<tr><th>代收点：</th><td><input type='checkbox' name='takeself' class='hide' value='0'/> <span id="takeself"></span>
								<a href="javascript:selectTakeself();"><span class="red">切换自提点</span></a>
								</td>
							</tr>
							<tr class='hide' id='takeselfParam'><td><%=province%></td><td><%=city%></td><td><%=area%></td></tr>
						</script>
					</table>
					<!--地址展示 结束-->
				...
				<div class="wrap_box" id='deliveryBox' style='display:none'>
					...
					<table width="100%" class="border_table m_10" id='delivery_form'>
						...
						<tbody>
							<?php $deliveryData = $this->allDeliveryType?>
							<?php foreach($deliveryData as $key => $item){?>
							<tr>
								<th><label><input type="radio" name="delivery_id" value="<?php echo isset($item['id'])?$item['id']:"";?>"  onclick='orderFormInstance.setDeliveryConf(<?php echo JSON::encode($item);?>);' /><?php echo isset($item['name'])?$item['name']:"";?></label></th>
								<td>
				...
				<div class="wrap_box" id='paymentBox' style='display:none'>
					...
					<table width="100%" class="border_table" id='payment_form'>
						...
						<?php if($item['id']!=0 || $this->freight_collect!=0){?>
						<tr>
				...
				<div class="wrap_box">
					...
					<div class="cart_prompt f14 t_l m_10" <?php if(empty($this->promotion)){?>style="display:none"<?php }?>>
						...
						<?php foreach($this->goodsList as $key => $value){?>
						...
						<table width="100%" class="cart_table t_c" >
							...
							<caption>商家：<?php echo isset($value['seller_name'])?$value['seller_name']:"";?></caption>
							<thead>
								<tr>
									<th>图片</th>
									<th>商品名称</th>
							...
							<tbody >
								...
								<tr>
									...
									<td id="deliveryFeeBox_<?php echo isset($item['goods_id'])?$item['goods_id']:"";?>_<?php echo isset($item['product_id'])?$item['product_id']:"";?>_<?php echo isset($item['count'])?$item['count']:"";?>"></td>
		...
		<div class="cart_box" id='amountBox' style='display:none'>
			<div class="cont_2">
				...
				<div class="pink_box gray m_10">
					<table width="100%" class="form_table t_l">
						...
						<tr>
							...
							<!--<label id="tax_title" class='attr' style='display:none'>发票抬头：<input type='text' class='normal' name='tax_title' /></label>-->
							</td>
					...
					<div id="tax_title"  style='display:none' class="attr contfp">
							<form action="" method="post">
								<div class="tylx">
									<span class="titles">发票类型：</span>
									<span>
										<input class="ptfps" type="radio" name="fp" value="radio1"/>普通发票
										<input class="zzsfp" type="radio" name="fp" value="radio2"/>增值税发票
									</span>
								</div>
								...
								<div class="anniu show1">
										<ul>
											<li><i>&nbsp;</i><p style="width:85px;">发票抬头：</p><input class="tex" name="" type="text"></li>
										</ul>
								</div>
								<!--普通发票结束-->
								<!--增值税发票-->
									<div class="anniu show2" style="display:none;">
										<ul>
											<li><i>*</i><p>单位名称：</p><input class="tex" name="" type="text"></li>
											<li><i>*</i><p class="shibh">纳税人识别号：</p><input class="tex" name="" type="text"></li>
											<li><i>*</i><p>注册地址：</p><input class="tex" name="" type="text"></li>
											<li><i>*</i><p>注册电话：</p><input class="tex" name="" type="text"></li>
											<li><i>*</i><p>开户银行：</p><input class="tex" name="" type="text"></li>
											<li><i>*</i><p>银行账户：</p><input class="tex" name="" type="text"></li>
										</ul>
									</div>
								...
								<div class="tylx">
									<input class="qer" type="button" value="确认发票信息"/>
				...
				<p class="m_10 t_r"><input type="submit" class="btn_red btn_order" value='确认无误，提交订单'/></p>
			</div>
		</div>
	</form>
</div>
...
_mvq.push(['$setGeneral', 'ordercreate', '', /*用户名*/ 'user', /*用户id*/ '21']);
_mvq.push(['$logConversion']);
_mvq.push(['$addOrder',/*订单号*/ '2341234234', /*订单金额*/ '222']);
_mvq.push(['$addItem', /*订单号*/ '2341234234', /*商品id*/ '31', /*商品名称*/ '汾酒', /*商品价格*/ '400', /*商品数量*/ '1', /*商品页url*/ 'http://121.40.61.222/site/products/id/31', /*商品页图片url*/ 'http://121.40.61.222/site/products/id/31']);
_mvq.push(['$logData']);
</script>
