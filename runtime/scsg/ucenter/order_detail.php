...
<div class="main f_r">
	...
	<div class="prompt_2 m_10">
        ...
        <p>
			...
			<label >
				<input type="button" class='btn_brown btn_small' value="立即付款" onclick="window.location.href='<?php echo IUrl::creatUrl("/block/doPay/order_id/".$this->order_info['order_id']."");?>'" />
			</label>
			<?php }?>
	        ...
	        <label >
	        	...
	        	<input type="submit" class='btn_gray btn_small' value="确认收货" />
	        </label>
			<?php }?>

	        <?php if(Order_Class::isRefundmentApply($this->order_info)){?>
	        <label >
	        	<input type="button"  class='btn_red btn_small' value="申请退换货" onclick='javascript:window.location.href="<?php echo IUrl::creatUrl("/ucenter/refunds_edit/order_id/".$this->order_info['order_id']."");?>"' />
	        </label>
	    	<?php }?>
	    </p>
        </form>
