...
<div class="ucenter container">
	...
	<div class="wrapper clearfix">
		...
		<script type="text/javascript" charset="UTF-8" src="/xinde/scsg/runtime/_systemjs/my97date/wdatepicker.js"></script>
<div class="main f_r">
	...
	<!--新增搜索-->
	<div class="sstj">
	<form action='<?php echo IUrl::creatUrl("/ucenter/order");?>' method='post' name='search_order'>
	...
	<form action='<?php echo IUrl::creatUrl("/block/dopaymerge");?>' method='post' name='merge_pay'>
...
<!--全选js和显示隐藏筛选条件的js写在nav.js中-->
    <div class="box m_10">
        <table class="list_table" width="100%" cellpadding="0" cellspacing="0">
            ...
            <?php $queryOrderList=$this->order_db?>
			<?php foreach($queryOrderList->find() as $key => $item){?>
			<?php $status=Order_Class::getOrderStatus($item);?>
            <tr>
				
                <td>
                	<?php if($status==2){?>
					<input  name="sub[]" class="checks" type="checkbox" value='<?php echo isset($item["id"])?$item["id"]:"";?>'/>
					<?php }?>
				...
				<td>
					<?php if($status==2){?>
                	<a href='<?php echo IUrl::creatUrl("/block/dopay/order_id/".$item["id"]."");?>' target=='blank'><input type="button" class='btn_gray btn_small' style='margin-bottom:5px;' value="立即支付" /></a>
					<a href="<?php echo IUrl::creatUrl("/ucenter/order_status/op/cancel/order_id/".$item['id']."");?>"><input type="button" class='btn_gray btn_small' value="取消订单" /></a>
					<?php }elseif($status==1){?>
					<a href="<?php echo IUrl::creatUrl("/ucenter/order_status/op/cancel/order_id/".$item['id']."");?>"><input type="button" class='btn_gray btn_small' value="取消订单" /></a>
					<?php }elseif(in_array($status,array(4,8,9,10,11))){?>
					<a href="<?php echo IUrl::creatUrl("/ucenter/refunds_edit/order_id/".$item['id']."");?>"><input type="button" class='btn_gray btn_small' value="申请退换货" /></a>
					<?php }elseif(in_array($status,array(3,11))){?>
					<a href="<?php echo IUrl::creatUrl("/ucenter/order_status/op/confirm/order_id/".$item['id']."");?>"><input style='margin-bottom:5px;' type="button" class='btn_gray btn_small' value="确认收货" /></a>
					<a href="<?php echo IUrl::creatUrl("/ucenter/refunds_edit/order_id/".$item['id']."");?>"><input type="button" class='btn_gray btn_small' value="申请退换货" /></a>
					
					<?php }elseif($status==6){?>
					<a href="<?php echo IUrl::creatUrl("/ucenter/evaluation");?>"><input type="button" class='btn_gray btn_small' value="评价" /></a>
					<?php }else{?>
					<b class="orange"><?php echo Order_Class::orderStatusText($status);?></b>
					<?php }?>
					<!--<img class="img_delt" src="<?php echo IUrl::creatUrl("")."views/".$this->theme."/skin/".$this->skin."/images/front/ucenter/bin_closed.png";?>"/>-->
					
                </td>
            <?php }?>
            </tr>
            <tfoot><tr><td colspan="6" class="t_l"><?php echo $this->order_db->getPageBar();?></td></tr></tfoot>
        </table>
    </div>
	</form>
</div>
...
formObj.init({
	'order_no':'<?php echo isset($s_order_no)?$s_order_no:"";?>',
	'beginTime':'<?php echo isset($s_beginTime)?$s_beginTime:"";?>',
	'endTime':'<?php echo isset($s_endTime)?$s_endTime:"";?>',
	'status':'<?php echo isset($s_status)?$s_status:"";?>',
	'seller_id':'<?php echo isset($s_seller_id)?$s_seller_id:"";?>',
});
<?php if($s_beginTime||$s_endTime||$s_status||$s_seller_id){?>
$('.jsx').show();
<?php }?>
</script>
