(function autoValidate()
{
    ...
    {    
        for(var i = 0; i < document.forms.length; i++)
        {
            ...
            {
                var e = f.elements[j];
                if(e.type != "text" && e.type!="password" && e.type!='select-one' && e.type!='textarea' && !e.getAttribute("js_require")) continue;
    ...
    function validateOnChange()
    {
		...
		switch(pattern)
		{
			...
			case 'mobile': pattern = /^1[2|3|4|5|6|7|8|9][0-9]\d{8}$/;break;
			...
			case 'intval':	pattern = /^[1-9]\d+$/i;break;
    ...
    function validateOnSubmit()
    {
        ...
        for(var i = 0; i < this.elements.length; i++)
        {
            ...
            if(((e.type == "text" || e.type == "password" || e.type == "select-one" || e.type == "textarea" ) && e.getAttribute("pattern") && e.style.display!='none' && e.offsetWidth > 0) || e.getAttribute('js_require'))
            {
				addEvent(e,'change',validateOnChange);
